...
class USVN_SVNUtils
{
    ...
    */
    ...
    /**
    * Configure svnserve authentication for a repository
    * This sets up the repository to use USVN's centralized passwd and authz files
    *
    * @param string Path to the repository
    */
    public static function configureSvnserveAuth($path)
    {
      $svnserveConf = $path . '/conf/svnserve.conf';
      if (!file_exists($svnserveConf)) {
        return; // Skip if conf file doesn't exist
      }
      
      // Get paths from USVN config
      try {
        $config = Zend_Registry::get('config');
        $authzPath = $config->subversion->authz;
        // Get passwd file path (same directory as authz)
        $passwdPath = dirname($authzPath) . '/passwd';
      } catch (Exception $e) {
        // Fallback to default paths if config not available
        $authzPath = '/volume/usvn/authz';
        $passwdPath = '/volume/usvn/passwd';
      }
      
      // Read the current configuration
      $content = file_get_contents($svnserveConf);
      
      // Configure authentication settings
      $content = preg_replace('/^# anon-access = read/m', 'anon-access = none', $content);
      $content = preg_replace('/^# auth-access = write/m', 'auth-access = write', $content);
      $content = preg_replace('/^# password-db = passwd/m', 'password-db = ' . $passwdPath, $content);
      $content = preg_replace('/^# authz-db = authz/m', 'authz-db = ' . $authzPath, $content);
      
      file_put_contents($svnserveConf, $content);
    }
