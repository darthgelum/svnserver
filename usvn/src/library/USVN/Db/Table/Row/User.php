...
{
	/**
	 * Temporary storage for plain text password before encryption
	 * @var string
	 */
	private $_plainTextPassword = null;
	...
	protected function _insert()
	{
		...
		// Store plain text password for svnserve before encryption
		$this->_plainTextPassword = $this->_data['users_password'];
	...
	{
		// Update htpasswd and passwd files with the new user
		if ($this->_plainTextPassword !== null) {
			$this->getTable()->updateHtpasswd(array(
				$this->_data['users_login'] => $this->_plainTextPassword
			));
		}
	}
	...
	{
		// Update htpasswd and passwd files if password changed
		if ($this->_plainTextPassword !== null) {
			$this->getTable()->updateHtpasswd(array(
				$this->_data['users_login'] => $this->_plainTextPassword
			));
